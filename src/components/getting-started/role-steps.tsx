...
import { BASE_IDENTITY_POLICY, ROLE_NAME_PREFIX } from '@/lib/aws/actions';
...
export async function RoleSteps() {
  const { t, rich, wizard, detail, checklist, openAdd, createValues, findRole } = await guideTranslators();
        ...
        <Tabs defaultValue="server" className="gap-5">
            ...
            <SubSteps
              items={[
                ...
                { content: rich('role.steps.s0.server.i3') },
                {
                  ...
                  extra: (
                    <>
                      <CodeBlock label={t('role.steps.s0.policyLabel')} value={basePolicy} />
          ...
          <TabsContent value="aws">
            <SubSteps
              items={[
                { content: findRole },
                { content: rich('role.steps.s0.aws.i2'), extra: <CodeBlock label={t('role.steps.s0.policyLabel')} value={basePolicy} /> },
            ...
            { content: rich('role.steps.s4.i1', createValues) },
            {
              ...
              extra: (
                <ul className="divide-y overflow-hidden rounded-xl border bg-card">
                  {(
                    [
                      ['green', TONE_DOT.success, checklist('statuses.ok')],
                      ['orange', TONE_DOT.warning, checklist('statuses.denied')],
                      ['red', TONE_DOT.danger, ''],
                    ] as const
                  ).map(([key, dot, status]) => (
