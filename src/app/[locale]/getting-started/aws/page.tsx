...
export const generateMetadata = localizedTitle('GettingStarted.aws.metaTitle');
 ...
 * The AWS guide.
 *
 * It was the whole of Get Started until OpsWatch grew a GitHub, a Cloudflare and an AI connection. The
 * content is unchanged: what moved is that it is now reached from a hub that asks what you want to connect
 * rather than being the thing "Get started" means.
 */
export default async function AwsGuidePage({ params }: Props) {
        ...
        <header className="rounded-2xl border bg-gradient-to-br from-primary/10 via-background to-background px-6 py-10 md:px-10">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Button asChild size="lg">
              <Link href="/accounts/new/aws">{t('cta')}</Link>
            </Button>
            <Button asChild size="lg" variant="outline">
              <Link href="/getting-started">{t('allGuides')}</Link>
            </Button>
          </div>
        </header>
