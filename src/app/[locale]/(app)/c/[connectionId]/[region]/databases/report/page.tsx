...
/** The databases report (§19): stored rollups only, and every figure against the period before. */
export default async function ReportPage({ params, searchParams }: Props) {
  ...
  return (
    <SectionLayout context={context} section="databases" subsection="report">
      <ReportView
        report={report}
        locale={context.locale}
        basePath={subsectionPath(context.scope, 'databases', 'report')}
        exportHref={`/api/v1/reports?env=${context.scope.connectionId}:${context.scope.region}&section=databases&period=${period}&format=markdown&locale=${context.locale}`}
      />
    </SectionLayout>
  );
}
