...
/** The overview report (§19): stored rollups only, and every figure against the period before. */
export default async function ReportPage({ params, searchParams }: Props) {
  ...
  const report = readReport(
    getDb(),
    { connectionId: context.scope.connectionId, scope: context.scope.region, section: 'overview', period },
    { nowMs: pageNow(), familyLabel: (family) => families(family) },
  );

  return (
    <SectionLayout context={context} section="overview" subsection="report">
      <ReportView
        report={report}
        locale={context.locale}
        basePath={subsectionPath(context.scope, 'overview', 'report')}
        exportHref={`/api/v1/reports?env=${context.scope.connectionId}:${context.scope.region}&section=overview&period=${period}&format=markdown&locale=${context.locale}`}
      />
    </SectionLayout>
  );
}
