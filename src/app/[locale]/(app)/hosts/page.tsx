...
export default async function HostsPage({ params }: Props) {
  ...
  const nowMs = pageNow();
  const hosts = listHosts(getDb(), nowMs);
            ...
            {hosts.map((host) => (
              <li key={host.id} className="flex flex-wrap items-start justify-between gap-x-4 gap-y-2 py-3 first:pt-0 last:pb-0">
                <div className="flex min-w-0 items-start gap-3">
                  <span className={cn('mt-1.5 size-2 shrink-0 rounded-full', STATE_DOT[host.state])} aria-hidden />
                  ...
                  {host.latest?.memoryTotalBytes !== null && host.latest?.memoryUsedBytes != null && (
                    <p className="mt-0.5">
                      {t('memoryOf', {
                        ...
                        total: formatMetricValue(host.latest.memoryTotalBytes ?? 0, 'bytes', locale),
                      })}
                    </p>
                  )}
                </div>
