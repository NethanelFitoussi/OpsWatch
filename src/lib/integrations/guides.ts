...
export const GUIDED = ['aws', 'gcp', 'do', 'github', 'cloudflare', 'ai'] as const satisfies readonly IntegrationId[];
...
export const GUIDE_CHAPTERS: Record<GuidedIntegration, { steps: readonly string[]; failures: readonly string[] }> = {
  aws: { steps: [], failures: [] },
  do: {
    steps: ['scope', 'create', 'paste', 'verify', 'droplets'],
    failures: ['unauthorized', 'forbidden', 'rate_limited', 'unreachable', 'no_token'],
  },
  gcp: {
