...
/** The range a page opens with when nothing else says otherwise; the Settings page replaces it. */
...
/** A page's range: its `?range=` when it names a known one, otherwise the configured default. */
export function parseTimeRange(value: string | string[] | undefined, fallback: TimeRange = DEFAULT_TIME_RANGE): TimeRange {
  const first = Array.isArray(value) ? value[0] : value;
  return isOneOf(TIME_RANGES, first) ? first : fallback;
}
