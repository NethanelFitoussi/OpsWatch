...
export const API_ROUTES: ApiRouteSpec[] = [
  ...
  },
  {
    method: 'post',
    path: '/ingest/host',
    operationId: 'ingestHostReport',
    auth: 'signature',
    summary: 'Where a Linux agent reports. Signed with the host’s own key; the host comes from the signature, never from the body.',
    request: hostReportSchema,
    response: hostAcceptedSchema,
    status: 202,
    errors: ['unauthorized', 'invalid_request', 'conflict', 'rate_limited'],
  },
  {
    method: 'get',
    path: '/investigations/{id}',
    pathParams: { id: 'The problem id. An investigation is derived from a problem, so it carries the problem’s id.' },
    operationId: 'getInvestigation',
    auth: 'session',
    summary: 'The investigation of one problem: observed facts, correlations and hypotheses, kept apart by `kind`.',
    response: investigationSchema,
    status: 200,
    errors: ['unauthorized', 'invalid_request', 'not_found'],
  },
  {
    ...
    auth: 'session',
    summary: 'The log groups one environment holds, narrowed by an optional name pattern.',
    response: logSourceListSchema,
    status: 200,
    errors: ['unauthorized', 'invalid_request', 'not_found', 'aws_denied', 'aws_throttled', 'aws_error'],
  },
  {
    method: 'post',
    path: '/logs/searches',
    operationId: 'startLogSearch',
    auth: 'session',
    summary: 'Starts a log search. The server composes the query; the answer is a search that has started, not its results.',
    request: logSearchRequestSchema,
    response: logSearchSchema,
    status: 202,
    errors: ['unauthorized', 'forbidden_origin', 'invalid_request', 'not_found', 'aws_denied', 'aws_throttled', 'aws_error'],
  },
  {
    method: 'get',
    path: '/logs/searches/{id}',
    pathParams: { id: 'The search id, as starting one reports it.' },
    operationId: 'getLogSearch',
    auth: 'session',
    summary: 'Where a search has got to, and its lines once it has any. `partial` means more matched than were returned.',
    response: logSearchSchema,
    status: 200,
    errors: ['unauthorized', 'invalid_request', 'not_found', 'aws_denied', 'aws_throttled', 'aws_error'],
  },
  {
    ...
    path: '/logs/searches/{id}',
    pathParams: { id: 'The search id, as starting one reports it.' },
    operationId: 'stopLogSearch',
    auth: 'session',
    summary: 'Stops a search that is still running, so a caller that gave up stops being billed for it.',
    status: 204,
    errors: ['unauthorized', 'forbidden_origin', 'invalid_request', 'not_found', 'aws_denied', 'aws_throttled', 'aws_error'],
  },
  {
