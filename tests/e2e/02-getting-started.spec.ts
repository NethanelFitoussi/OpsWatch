...
test('switching to French persists across visits', async ({ page }) => {
  // The hub, because that is where the root redirects and what the locale switch has to preserve.
  ...
  await expect(page.getByRole('heading', { level: 1, name: 'Connectez les systèmes que vous utilisez' })).toBeVisible();
