...
| Remaining | |
|---|---|
| Host findings as first-class problems | `PARTIAL` — a machine's findings now appear on the **Health page of the account and region it was placed in**, read at render from its last reading. Deliberately not written into `problems`: that table is keyed to an AWS environment by a column that cannot be null, and making it nullable turns every scoped read into a tri-state where one missed predicate leaks or hides rows — against the isolation work of `cd78688`. The card says what it is not, because one that looked like the Problems list would promise a notification that is not coming. **Still missing: alerting.** `alert_rules` is scoped to `(connection, region)` too, so a machine's trouble is visible and not yet notifiable |
