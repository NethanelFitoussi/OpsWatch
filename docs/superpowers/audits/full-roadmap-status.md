...
| MC-9 | Push collection is pinned to `regions[0]`: `aws_collection` holds one forwarder per connection, but a subscription filter can only target a Lambda in its own region | `PARTIAL` — the page now **says which region** the stack forwards from and names the regions it does not, so nobody waits for logs that can never arrive. Making it genuinely per region is a schema change: the consent (`managed`, `realtimeLogs`, `persistLogs`, the ingest secret) belongs to the account, the stack identity (`stackName`, `forwarderArn`, `verifiedAt`) belongs to a region |
