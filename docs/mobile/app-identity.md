...
`com.example.*`, which both stores reject.
...
| Associated domain | unset | `OPSWATCH_ASSOCIATED_DOMAIN` | Yes |
| Version | `0.1.0` | `VERSION` in `app.config.ts` + `package.json` | Every release |
| iOS build number | `1` | `OPSWATCH_IOS_BUILD_NUMBER` or EAS autoIncrement | Must increase every upload |
| Android versionCode | `1` | `OPSWATCH_ANDROID_VERSION_CODE` or EAS autoIncrement | Must increase every upload |
...
- iOS `buildNumber` and Android `versionCode`: integers that must increase with every store upload.
